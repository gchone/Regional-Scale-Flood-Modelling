    ...
    class CreatePointsAlongReaches(QgsProcessingAlgorithm):
        ...
        def shortHelpString(self):
            return (
                ...
                "point locations. Used in Extract Water Surface and Width workflows.\n\n"
                "Inputs:\n"
                "- Route feature class: oriented route network with M-values (e.g. routes_main)\n"
                "- Route links table: DownID/UpID link table (e.g. routes_main_links)\n"
                "- RouteID field: RID\n"
                "- Interval: distance between points in metres (e.g. 5m for water surface, "
                "200 or 500m for width depending on river size)\n\n"
                "Output:\n"
                "- Points along route (e.g. target_pts, width_pts) with fields id, RID, MEAS\n"
                "  Note for Extract Water Surface workflow: after running, manually delete points outside the DEM and under bridges.\n"
            )
        ...
        def initAlgorithm(self, config=None):
            ...
            self.addParameter(QgsProcessingParameterVectorLayer(
                self.ROUTES,
                "Route feature class (e.g. routes_main)",
                [QgsProcessing.TypeVectorLine],
            ))
            self.addParameter(QgsProcessingParameterVectorLayer(
                self.LINKS,
                "Route links table (e.g. routes_main_links)",
            ...
            self.addParameter(QgsProcessingParameterNumber(
                self.INTERVAL,
                "Interval between points in metres (e.g. 5 for water surface, 200 or 500 for width)",
                type=QgsProcessingParameterNumber.Double,
                defaultValue=5.0,
            ))
            self.addParameter(QgsProcessingParameterFeatureSink(
                self.OUTPUT,
                "Points along route (e.g. target_pts, width_pts)",
            ))
