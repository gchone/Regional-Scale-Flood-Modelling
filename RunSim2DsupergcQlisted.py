            ...
            for point in sorted(segment, key=lambda q: q[2]):
                ...
                if point[3]=="main" and not skipsim:
                    ...
                    if not arcpy.Exists(currentsimfolder + "\\elev_zone" + str(point[1])):
                        ...
                        # faire une copie du bci dans le dossier du debit
                        # ouvrir en mode append le nouveau fichier bci
                        # il faudra aussi modifier le fichier par
                        # fermer le fichier bci à la fin
                        ...
                        ### Début de traitement pour la configuration de la fenêtre de sortie ###
                        numhvar = 1
                        newpoint = pointflowpath()
                        newpoint.side = listzonesout[point[1]][1]
                        print(newpoint.side)
                        newpoint.shape = listzonesout[point[1]][2].firstPoint
                        colinc = 0
                        rowinc = 0
                        distinc = 0
                        newpoint.side2 = "0"
                        ...
                        # Lecture de l'élévation du point de sortie
                        if not lakebci:
                            hfix = res_downstream.getValue(res_downstream.YtoRow(newpoint.shape.Y), res_downstream.XtoCol(newpoint.shape.X))
                            if hfix != res_downstream.nodata:
                                # Ajout d'une ligne dans le bci
                                filebci.write(
                        ...
                        # On progresse dans une direction jusqu'à sortir du raster ou jusqu'à ce que la distance voullue soit attente
                        while (not (
                            ...
                            if not lakebci:
                                ...
                                current_X = ref_raster.ColtoX(currentcol)
                                current_Y = ref_raster.RowtoY(currentrow)
                                hfix = res_downstream.getValue(res_downstream.YtoRow(current_Y), res_downstream.XtoCol(current_X))
                                ...
                                if hfix != res_downstream.nodata:
                                    ...
                                    # Ajout dans le bdy
                                    zdep = ref_raster.getValue(currentrow, currentcol)
                                    filebdy.write("\nhvar" + str(numhvar) + "\n")
                                    filebdy.write("4\tseconds\n")
                                    filebdy.write("{0:.2f}".format(zdep) + "\t0\n")
                                    filebdy.write("{0:.2f}".format(zdep) + "\t50000\n")
                        ...
                        if distance < distoutput / 2:
                            ...
                            if newpoint.side == "W":
                                ...
                                colinc = 0
                                distinc = ref_raster.raster.meanCellHeight
                                point.lim3 = max(ref_raster.raster.extent.YMin, ref_raster.raster.extent.YMax - (
                            ...
                            while (not (
                                ...
                                if not lakebci:
                                    current_X = ref_raster.ColtoX(currentcol)
                                    current_Y = ref_raster.RowtoY(currentrow)
                                    hfix = res_downstream.getValue(res_downstream.YtoRow(current_Y),
                                                                   res_downstream.XtoCol(current_X))
                        ...
                        # Cas des lacs: la condition limite aval est définie sur l'ensemble de la distance distoutput
                        if lakebci:
                            # Ajout d'une ligne dans le bci
                            filebci.write(
